layui.use(['layer', 'element', 'util','element'], function () {
    ...
    element.on('nav(side-top-right)', function (elem) {
        // 修改skin
        if (elem['context'].parentNode.dataset.skin) {
            localStorage.skin = elem['context'].parentNode.dataset.skin;
            skin();
        } else {
            // 添加tab方法
            window.addTab(elem);
    ...
    window.getThisTabID = function () {
        // 当前选中的选项卡id
        return $(document).find('.LAY-body-tm .my-body .layui-tab-card > .layui-tab-title .layui-this').attr('lay-id');
    };
